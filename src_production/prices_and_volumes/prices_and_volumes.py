...
#     all_bars = []
def descargar_ticker(ticker, start, end, api_key, api_secret):
    url = "https://data.alpaca.markets/v2/stocks/bars"
    headers = {"APCA-API-KEY-ID": api_key, "APCA-API-SECRET-KEY": api_secret}
    params = {"symbols": ticker, "timeframe": "1Min", "start": start, "end": end, "feed": "sip", "adjustment": "all"}
...
def handler(event, context):
    ...
    START_HOY = apertura_ny.astimezone(pytz.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    END_HOY = cierre_ny.astimezone(pytz.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    for i, ticker in enumerate(lista_tickers, 1):
        ...
        bars = descargar_ticker(ticker, START_HOY, END_HOY, API_KEY, API_SECRET)
        if bars:
